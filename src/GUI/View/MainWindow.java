...
public class MainWindow<T extends IRecord<T> & IHashable> extends JFrame {
    ...
    private void showBuckets() {
        ...
        int n = this.file.getPrimaryFile().getTotalBlocks();
        for (int i = 0; i < n; i++) {
            this.outputArea.append("Bucket " + i + ":\n");

            int cur = i;

            ChainedBlock<T> b = this.file.getPrimaryFile().getBlock(cur);
            this.outputArea.append("Primary file Block " + cur + ", NextBlockIndex: " + b.getNextBlockIndex() + " (" + b.getValidCount() + " records):\n");
            ...
            while (cur != -1) {
                ...
                this.outputArea.append("   Overflow file Block " + cur + ", NextBlockIndex: " + b.getNextBlockIndex() +  " (" + b.getValidCount() + " records):\n");
                for (int j = 0; j < b.getValidCount(); j++) {
                    this.outputArea.append("    " + b.getRecordAt(j) + "\n");
                }
                cur = b.getNextBlockIndex();
